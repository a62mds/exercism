...
class PlayingCardTest(unittest.TestCase):
    def test_a_card(self):
        fH = PlayingCard('4H')
        self.assertEqual(fH.rank, '4')
        self.assertEqual(fH.suit, 'H')

    def test_invalid_rank(self):
        with self.assertRaises(ValueError) as context:
            NaC = PlayingCard('BH')
        self.assertTrue('Invalid card rank: B' in context.exception)

    def test_invalid_suit(self):
        with self.assertRaises(ValueError) as context:
            NaC = PlayingCard('JK')
        self.assertTrue('Invalid card suit: K' in context.exception)

    def test_invalid_rank_and_suit(self):
        with self.assertRaises(ValueError) as context:
            NaC = PlayingCard('@B')
        self.assertTrue('Invalid card rank: @' in context.exception)


class PokerHandTest(unittest.TestCase):
    poker_hands = {'straight flush'  : StraightFlush.is_straight_flush,
                   'four of a kind'  : FourOfAKind.is_four_of_a_kind,
                   'full house'      : FullHouse.is_full_house,
                   'flush'           : Flush.is_flush,
                   'straight'        : Straight.is_straight,
                   'three of a kind' : ThreeOfAKind.is_three_of_a_kind,
                   'two pair'        : TwoPair.is_two_pair,
                   'one pair'        : OnePair.is_one_pair,
                   'nothing'         : Nothing.is_nothing}

    def generic_hand_test(self, card_string, name, *also_allowed):
        card_names = card_string.split()
        the_hand = PokerHandFactory(card_names)
        hand_names = set([name] + [n for n in also_allowed])
        self.assertTrue(PokerHandTest.poker_hands[name](the_hand.cards))
        self.assertEqual(the_hand.name, name)
        self.assertEqual(str(the_hand), card_string)
        self.assertEqual(the_hand.card_strs(), card_names)
        for hand in set(PokerHandTest.poker_hands.keys()) - hand_names:
            self.assertFalse(PokerHandTest.poker_hands[hand](the_hand.cards))

    def test_straight_flush(self):
        self.generic_hand_test('KH QH JH 10H 9H', 'straight flush', 'straight', 'flush')

    def test_four_of_a_kind(self):
        self.generic_hand_test('KH KC KS KD JH', 'four of a kind')

    def test_full_house(self):
        self.generic_hand_test('KH KC KS QH QC', 'full house')

    def test_flush(self):
        self.generic_hand_test('KH QH 10H 9H 8H', 'flush')

    def test_straight(self):
        self.generic_hand_test('2H 3C 4H 5D 6S', 'straight')

    def test_three_of_a_kind(self):
        self.generic_hand_test('KH KC KS QH JH', 'three of a kind')

    def test_two_pair(self):
        self.generic_hand_test('KH KC QH QC JH', 'two pair')

    def test_one_pair(self):
        self.generic_hand_test('KH KC 2H 4S 5D', 'one pair')

    def test_nothing(self):
        self.generic_hand_test('2H 9C 6D 4C QH', 'nothing')
